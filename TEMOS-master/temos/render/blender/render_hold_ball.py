...
from .floor import show_traj, plot_floor, get_trajectory, plot_floor_for_hold_ball
...
def render_hold_ball(npydata, frames_folder, *, mode, faces_path, gt=False,
    ...
    # Create a floor
    plot_floor_for_hold_ball(data.data, big_plane=False)
    ...
    for index, frameidx in enumerate(frameidx):
        ...
        sphere_center = (left_hand + right_hand) / 2
        print("sphere_center = ", sphere_center, frameidx)
        bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=3, radius=0.2, location=(sphere_center[0], sphere_center[1], sphere_center[2]))   
