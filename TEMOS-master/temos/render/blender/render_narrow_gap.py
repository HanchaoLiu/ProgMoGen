...
from .floor import show_traj, plot_floor, get_trajectory, plot_floor_for_narrow_gap
...
from .materials import body_material, body_material_transparent_with_alpha
...
def render_narrow_gap(npydata, frames_folder, *, mode, faces_path, gt=False,
    ...
    # Create a floor
    plot_floor_for_narrow_gap(data.data, big_plane=False)
    ...
    if True:
        ...
        # z_height_2 = 0.55
        z_height_2 = 0.25
        location = (s,-1,z_height_2)
        scale = (1, 2, z_height_2)
        bpy.ops.mesh.primitive_plane_add(size=2, enter_editmode=False, align='WORLD', location=location, scale=(1,1,1), rotation=(0.0, np.pi/2, 0.0))
        bpy.ops.transform.resize(value=scale, orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL',
                                constraint_axis=(False, True, False), mirror=True, use_proportional_edit=False,
                                proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False,
    ...
    # for head stretch wide 
    # frameidx = [0,30,60]

    # for head stretch not wide but cross
    if mode=="sequence":
        frameidx = [0,25,60]
    else:
        # frameidx = list(range(60))    
        frameidx = list(range(150))    
