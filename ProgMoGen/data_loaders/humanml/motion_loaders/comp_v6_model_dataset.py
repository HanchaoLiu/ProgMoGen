...
class CompMDMGeneratedDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.dataset.load_mode == 'eval' and self.dataset.dataset_name != 'babel':  # re-norm is not needed for babel dataset
            ...
            # this is used!
            # print("check whether mean_for_eval is used!")

        # print("hello_right place!!!!!!!!!!!!!!!!!!!!!!!!!!")
...
class CompMDMInpaintingGeneratedDataset(CompMDMGeneratedDataset):
    def __init__(self, args, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale=1.):
        ...
        print("final len(generated_motion)=", len(generated_motion))
...
class CompMDMInpaintingGeneratedDataset_debug(CompMDMGeneratedDataset):
    def __init__(self, args, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale=1., use_ddim_tag=False, 
        ...
        # use_ddim = False  # FIXME - hardcoded
        # diffusion.num_timesteps=100
        if use_ddim_tag==True:
            print("using ddim ")
            use_ddim = True  # FIXME - hardcoded
        else:
            print("not using ddim, using ddpm")
            use_ddim = False
        # diffusion.num_timesteps=100
        ...
        if True:
            for i, (motion, model_kwargs) in tqdm(enumerate(dataloader)):
                ...
                for t in range(repeat_times):
                    ...
                    if init_motion_type=='fullmotion':
                        sample = sample_fn(
                            model,
                            motion.shape,
                            clip_denoised=clip_denoised,
                    ...
                    elif init_motion_type=='partmotion':
                        part_motion = model_kwargs['y']['inpainted_motion'] * model_kwargs['y']['inpainting_mask']
                        sample = sample_fn(
                            model,
                            motion.shape,
                            clip_denoised=clip_denoised,
                    ...
                    elif init_motion_type=='plane':

                        # sample_fn = diffusion.ddim_sample_loop_opt_plane_relax_constrain_for_eval
                        # sample_fn = diffusion.ddim_sample_loop_opt_plane_relax_constrain_for_eval2
                        sample_fn = diffusion.ddim_sample_loop_opt_plane_relax_constrain_for_eval3
                        # diffusion.np_seed=0
                        ...
                        for i in range(bs):
                            # print(i,bs)
                            diffusion.np_seed = np.random.randint(0,1000)
                            model_kwargs_each = self.get_slice_model_kwargs(model_kwargs, i)
    ...
    def get_slice_model_kwargs(self, model_kwargs, i):
        res={}
        res['y']={}
        for k,v in model_kwargs['y'].items():
            res['y'][k] = v[i:i+1]
