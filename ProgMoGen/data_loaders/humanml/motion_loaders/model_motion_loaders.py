...
from data_loaders.humanml.motion_loaders.comp_v6_model_dataset import CompMDMGeneratedDataset, CompMDMInpaintingGeneratedDataset, \
    CompMDMUnfoldingGeneratedDataset, CompMDMInpaintingGeneratedDataset_debug
...
def get_mdm_loader(args, model, diffusion, batch_size, ground_truth_loader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale, num_unfoldings=0):
    ...
    if hasattr(args, "inpainting_mask") and args.inpainting_mask != '':
        ...
        dataset = CompMDMInpaintingGeneratedDataset_debug(args, model, diffusion, ground_truth_loader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale)
    ...
    motion_loader = DataLoader(dataset, batch_size=batch_size, collate_fn=collate_fn, drop_last=True, num_workers=1)
    mm_motion_loader = DataLoader(mm_dataset, batch_size=1, num_workers=1)
...
# our loader
# add use_ddim_tag
def get_mdm_loader_debug(args, model, diffusion, batch_size, ground_truth_loader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale, num_unfoldings=0, 
                         use_ddim_tag=False, init_motion_type='none'):
    opt = {
        'name': 'test',  # FIXME
    }
    print('Generating %s ...' % opt['name'])
    ...
    if hasattr(args, "inpainting_mask") and args.inpainting_mask != '':
        ...
        dataset = CompMDMInpaintingGeneratedDataset_debug(args, model, diffusion, ground_truth_loader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale, use_ddim_tag=use_ddim_tag,
                                                          init_motion_type=init_motion_type)
    elif num_unfoldings > 1:
        dataset = CompMDMUnfoldingGeneratedDataset(args, model, diffusion, ground_truth_loader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale, num_unfoldings)
    else:
        print("!!!!!!!!!else")
