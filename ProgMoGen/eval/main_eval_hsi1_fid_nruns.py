...
def f_add_args(parser):
    parser.add_argument("--use_ddim_tag", default=0, type=int, help="Use cuda device, otherwise use CPU.")
    parser.add_argument("--save_tag", default='', type=str, help="Use cuda device, otherwise use CPU.")
    parser.add_argument("--mask_type", default='', type=str, help="Use cuda device, otherwise use CPU.")
    parser.add_argument("--traj_method", default='', type=str, help="Use cuda device, otherwise use CPU.")
    parser.add_argument("--init_motion_type", default='', type=str, help="Use cuda device, otherwise use CPU.")
    parser.add_argument("--save_fig_dir", default='', type=str, help="Use cuda device, otherwise use CPU.")
    parser.add_argument("--ret_type", default='', type=str, help="Use cuda device, otherwise use CPU.")
    parser.add_argument("--text_split", default='', type=str, help="Use cuda device, otherwise use CPU.")
    parser.add_argument("--num_samples_limit", default=0, type=int, help="Use cuda device, otherwise use CPU.")

    parser.add_argument("--task_config", default='none', type=str, help="task configs")

    parser.add_argument("--diffusion_type", default='none', type=str, help="Use cuda device, otherwise use CPU.")
    parser.add_argument("--input_path", default='none', type=str, help="")
    return parser
        ...
        for i, (motion, model_kwargs) in enumerate(dataloader):
            ...
            for t in range(repeat_times):
                ...
                for ii in range(bs):
                    ...
                    # get h_list according to gt motion
...
if __name__ == '__main__':
    ...
    for ii in range(replication_times):
        ...
        motion_gen = torch.FloatTensor(file_data["motion_gen"])
        length_gen = torch.LongTensor(file_data["lengths"])
        ...
        res_list.append({'gt_mu':gt_mu, 'gt_cov':gt_cov})
