from diffusion.respace_v2 import SpacedDiffusion_v2
...
class InpaintingGaussianDiffusion(SpacedDiffusion_v2):
    ...
    def ddim_sample_loop_opt_fn(
        ...
        print(f"lr={base_lr},iterations={max_it}")
        ...
        for it in range(max_it):
            ...
            self.adjust_learning_rate(optimizer,base_lr,it,step=decay_steps)
            ...
            # optimization loss
            # joints = self.sample_to_joints(pred_res)
            loss = self.f_loss(pred_res, pred_res_0, it)

            # evaluation loss
            loss_eval = self.f_eval(pred_res.detach(), pred_res_0).mean()

            # print(f"{it}, lr={lr_curr:.4f}, loss_all={loss.item():.6f}, loss_reg={loss_reg.item():.6f}, loss_eval={loss_eval.item():.6f}")
        ...
        loss_confined_space = self.f_eval(pred_res_ret, pred_res_0)
        self.loss_ret_val = loss_confined_space.data.cpu().numpy()
    ...
    def ddim_sample_loop_opt_fn_goal(
        self,
        model,
        shape,
        noise=None,
        ...
        for it in range(max_it):
            ...
            # optimization loss
            # joints = self.sample_to_joints(pred_res)
            loss = self.f_loss(pred_res, pred_res_0, it, self.target_gt)

            # evaluation loss
            loss_eval = self.f_eval(pred_res.detach(), pred_res_0, self.target_gt).mean()
        ...
        # set loss eval
        # joints = self.sample_to_joints(pred_res_ret)
        loss_confined_space = self.f_eval(pred_res_ret, pred_res_0, self.target_gt)
        self.loss_ret_val = loss_confined_space.data.cpu().numpy()
