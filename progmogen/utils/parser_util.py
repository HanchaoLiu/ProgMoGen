...
def edit_inpainting_args():
    ...
    parser.add_argument("--dim", default='none', type=str, help="Use cuda device, otherwise use CPU.")
    parser.add_argument("--input_ref_data", default='', type=str, help="Use cuda device, otherwise use CPU.")
    parser.add_argument("--inpainting_method", default='', type=str, help="Use cuda device, otherwise use CPU.")
    return parse_and_load_from_multiple_models(parser, task='inpainting')


def edit_inpainting_args_add_args(f_add_args=None):
    parser = ArgumentParser()
    # args specified by the user: (all other will be loaded from the model)
    add_base_options(parser)
    add_sampling_options(parser)
...
def evaluation_inpainting_parser_add_args(f_add_args=None):
    parser = ArgumentParser()
    # args specified by the user: (all other will be loaded from the model)
    add_base_options(parser)
    add_evaluation_options(parser)
