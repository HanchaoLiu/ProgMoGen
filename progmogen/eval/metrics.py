...
def calculate_trajectory_error(dist_error, strict=True):
    ...
    all_fail_02 = (dist_error > 0.2).sum() / dist_error.numel()
    all_fail_05 = (dist_error > 0.5).sum() / dist_error.numel()
    ...
    return np.array([traj_fail_02, traj_fail_05, all_fail_02, all_fail_05, dist_error.mean()])
...
def calculate_foot_contact_ratio(motions):
    thresh_height = 0.05 # 10
    # thresh_height = 0.1
    # thresh_height = 0.2
    fps = 20.0
    ...
    feet_contact = np.logical_and((verts_feet_height[:, :, :-1] < thresh_height), (verts_feet_height[:, :, 1:] < thresh_height))  # [bs, 2, max_len - 1]
    # skate velocity
    # print("feet contact = ", feet_contact.shape)
    # (1,2,len)
