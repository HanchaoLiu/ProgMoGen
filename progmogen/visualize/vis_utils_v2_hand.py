...
from model.rotation2xyz_my import Rotation2xyz
import numpy as np
...
class npy2obj:
    def __init__(self, npy_path, sample_idx, rep_idx, device=0, cuda=True):
        ...
        if self.nfeats == 3:
            ...
            if False:
                # (1, 25, 6, 196)
                hand_list = [20,21,22,23]
                self.motions['motion'][:,hand_list,0,:]=1.0
                self.motions['motion'][:,hand_list,1,:]=0.0
                self.motions['motion'][:,hand_list,2,:]=0.0
                self.motions['motion'][:,hand_list,3,:]=0.0
                self.motions['motion'][:,hand_list,4,:]=1.0
                self.motions['motion'][:,hand_list,5,:]=0.0

                # self.motions['motion'][:,hand_list,0,:]=0.707
                # self.motions['motion'][:,hand_list,1,:]=-0.707
                # self.motions['motion'][:,hand_list,2,:]=0.0
                # self.motions['motion'][:,hand_list,3,:]=0.0
                # self.motions['motion'][:,hand_list,4,:]=1.0
                # self.motions['motion'][:,hand_list,5,:]=0.0
                ...
                hand_list=[22,20]
                self.motions['motion'][:,hand_list,0,:]=0.866
                self.motions['motion'][:,hand_list,1,:]=-0.5
                self.motions['motion'][:,hand_list,2,:]=0.0
                self.motions['motion'][:,hand_list,3,:]=0.0
                self.motions['motion'][:,hand_list,4,:]=1.0
                self.motions['motion'][:,hand_list,5,:]=0.0
        ...
        self.joints = self.rot2xyz(torch.tensor(self.motions['motion']), mask=None,
                                     pose_rep='rot6d', translation=True, glob=True,
                                    #  jointstype='vertices',
                                     jointstype='smpl',  # for joint locations
                                     vertstrans=True)
    ...
    def save_npy_numpy(self, save_path):
        # original vertices is torch.tensor, change to np.ndarray
        data_dict = {
            'motion': self.motions['motion'][0, :, :, :self.real_num_frames],
            'thetas': self.motions['motion'][0, :-1, :, :self.real_num_frames],
            'root_translation': self.motions['motion'][0, -1, :3, :self.real_num_frames],
    ...
    def save_npy_numpy_with_joints(self, save_path):
        # original vertices is torch.tensor, change to np.ndarray
        data_dict = {
            'motion': self.motions['motion'][0, :, :, :self.real_num_frames],
            'thetas': self.motions['motion'][0, :-1, :, :self.real_num_frames],
            ...
            'text': self.motions['text'][0],
            'length': self.real_num_frames,
            'joints': self.joints[0, :, :, :self.real_num_frames].numpy(),
        }
        np.save(save_path, data_dict)
