...
def recover_from_rot_with_skeleton(data, joints_num, skeleton, skel_joints):
    '''
    skel_joints: (bs, n_joints, 3)
    '''
    r_rot_quat, r_pos = recover_root_rot_pos(data)
    ...
    # print('skel_joints = ', skel_joints.shape)
    positions = skeleton.forward_kinematics_cont6d(cont6d_params, r_pos, skel_joints=skel_joints)
...
def reverse_pose(data, joints_num, joints_new):
    '''
    data: [bs, 1, 196, 263]
    ...
    joints_new = joints_new[None]

    r_rot_quat, r_pos = recover_root_rot_pos(data)
    # positions = data[..., 4:(joints_num - 1) * 3 + 4]
    # positions = positions.view(positions.shape[:-1] + (-1, 3))

    joints_new[..., 0] -= r_pos[..., 0:1]
    joints_new[..., 2] -= r_pos[..., 2:3]

    joints_new = qrot((r_rot_quat[..., None, :]).expand(joints_new.shape[:-1] + (4,)), joints_new)

    # print("positions=", positions.shape, "joints_new=",joints_new.shape)
    # assert np.allclose(positions, joints_new[:,:,:,1:,:])

    # torch.Size([1, 1, 196, 21, 3])
    local_pose = joints_new[:,:,:,1:,:]
    bs=local_pose.shape[0]*local_pose.shape[1]
    local_pose = local_pose.reshape(*(bs,)+local_pose.shape[2:])
    return local_pose
