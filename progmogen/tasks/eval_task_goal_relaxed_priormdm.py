...
def get_gen_motion(args, model, model_2, dataloader, num_samples_limit, scale, init_motion_type):
...
if __name__ == '__main__':
    ...
    if use_model=="left_wrist":
        ...
        args_list_2[0].model_path = os.path.join(priormdm_dir, 'left_wrist_finetuned/model000280000.pt')
        from diffusion.ddim_relax_inpaint import InpaintingGaussianDiffusion as InpaintingGaussianDiffusion_2
        DiffusionClass_2 = InpaintingGaussianDiffusion_2
        model_2, diffusion_2 = load_model_blending_and_diffusion(args_list_2, gen_loader, dist_util.dev(), DiffusionClass=DiffusionClass_2)
    elif use_model=="root_and_left_wrist":
        args_list_2 = copy.deepcopy(args_list)

        args_list_2 = [ copy.deepcopy(args_list_2[0]), copy.deepcopy(args_list_2[0])]
        args_list_2[0].model_path = os.path.join(priormdm_dir, 'left_wrist_finetuned/model000280000.pt')
        args_list_2[1].model_path = os.path.join(priormdm_dir, 'root_horizontal_finetuned/model000280000.pt')
    ...
    for ii in range(replication_times):
        ...
        # generate motions
        motion_gen_all, length_gen, texts_gen ,(caption_list, tokens_list, cap_len_list)  = get_gen_motion(args, model, model_2, gen_loader, num_samples_limit, args.guidance_param,
