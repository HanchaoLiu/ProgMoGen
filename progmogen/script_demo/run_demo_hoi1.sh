...
save_tag="move_object"

save_fig_dir="result/demo/${save_tag}_n${num_samples_limit}/${eval_method}_${ret_type}_npy"

task_config="task_hoi1_config"


###############################################################################
# generate motion
###############################################################################
python3 tasks/run_demo_goal_simple.py \
    --use_ddim_tag 1 \
...
${blender_app} --background --python render_demo_hoi1.py -- npy=${mesh_file} +npy_joint=${input_joint_file} +npy_joint_idx=${idx} canonicalize=true mode="sequence"
${blender_app} --background --python render_demo_hoi1.py -- npy=${mesh_file} +npy_joint=${input_joint_file} +npy_joint_idx=${idx} canonicalize=true mode="video"
